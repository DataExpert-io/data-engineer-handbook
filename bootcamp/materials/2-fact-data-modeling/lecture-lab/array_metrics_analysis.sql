...
SELECT
    ...
    CASE
        ...
        WHEN ya.metric_array IS NULL THEN
            ARRAY_FILL(
                0, ARRAY[COALESCE(date - DATE(DATE_TRUNC('month', date)), 0)]
            )
...
WITH agg AS (
    SELECT
        ...
        month_start,
        ARRAY[
            SUM(metric_array[1]), SUM(metric_array[2]), SUM(metric_array[3])
        ] AS summed_array
    FROM array_metrics
...
SELECT
    ...
    agg.elem AS value,
    agg.month_start
    + CAST(CAST(agg.index - 1 AS TEXT) || ' day' AS INTERVAL) AS adjusted_date
FROM agg
