/* In the lab, we didn't exactly use this query as
 we used the attribute years_since_last_season which is more informative
 than the is_active attribute defined here. */
WITH last_season AS (
    SELECT *
    FROM players
...
SELECT COALESCE(ls.player_name, ts.player_name) as player_name,
    ...
    COALESCE(
        ...
        WHEN ts.season IS NOT NULL THEN ARRAY [ROW(
                ts.season,
                ts.pts,
                ts.ast,
