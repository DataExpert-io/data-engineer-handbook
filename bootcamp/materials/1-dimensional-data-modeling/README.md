...
2. **Throwing errors like connection refused even when all details are correct.**
   - Please check that the port(5432 or port you use) runs only docker service, not any other services.
     - Windows - use the command to check port availability in cmd `netstat -ano | findstr :5432` - to kill rest of extra services use `taskkill /PID <PID> /F`
     - Mac - use the command on terminal to check port availability `lsof -i :5432` - to kill rest of extra services use `kill -9 <PID>`
